...
namespace Vortex.Growl.AppBridge
{
    internal sealed class DisplayStyleManager
    {
        ...
        public static Dictionary<string, Vortex.Growl.AppBridge.Display> GetAvailableDisplayStyles()
		{
            Dictionary<string, Vortex.Growl.AppBridge.Display> displayStyles = new Dictionary<string, Vortex.Growl.AppBridge.Display>();
            foreach (LoadedDisplayStyle loadedDisplayStyle in currentlyLoadedDisplayStyles.Values)
            {
                string[] displays = loadedDisplayStyle.Display.GetListOfAvailableDisplays();
                foreach (string name in displays)
                {
                    Vortex.Growl.AppBridge.Display display = new Vortex.Growl.AppBridge.Display(name, loadedDisplayStyle.Display);
