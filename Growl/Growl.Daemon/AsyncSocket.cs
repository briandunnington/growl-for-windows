    ...
    public class AsyncSocket
    {
        ...
        protected virtual bool OnSocketDidReadTimeout()
        {
            ...
            if (DidReadTimeout != null)
            {
                ...
                {
                    object[] args = { this };
                    result = (bool) synchronizingObject.Invoke(new DoDidReadTimeoutDelegate(DidReadTimeout), args);
                }
                else if (allowApplicationForms)
                {
                    System.Windows.Forms.Form appForm = GetApplicationForm();
                    if (appForm != null)
                    {
                        result = (bool) appForm.Invoke(new DoDidReadTimeoutDelegate(DidReadTimeout), this);
                    }
                }
                else if (allowMultithreadedCallbacks)
                {
                    // we cant support this since we need the return value
                }
            }
            return result;
        }
