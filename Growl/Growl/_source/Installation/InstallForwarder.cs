...
{
    public partial class InstallForwarder : Form
    {
        ...
        public InstallForwarder()
        {
            InitializeComponent();

            // localize text
            this.Text = Properties.Resources.ForwarderInstaller_FormTitle;
        ...
        public bool LaunchInstaller(string uri, bool appIsAlreadyRunning, ref List<InternalNotification> queuedNotifications, ref int cultureCodeHash)
        {
            ...
            try
            {
                ...
                ForwarderInfo info = ForwarderInfo.Parse(definition);
                if (info != null)
                {
                    this.InfoLabel.Text = String.Format(Utility.GetResourceString(Properties.Resources.ForwarderInstaller_Prompt), info.Name, info.Author, info.Description);
                    ...
                    if (result == DialogResult.Yes)
                    {
                        ...
                        Utility.WriteDebugInfo(String.Format("Downloading forwarder plugin '{0}' to {1}", info.Name, info.LocalZipFileLocation));
                        ...
                        Utility.WriteDebugInfo(String.Format("Finished downloading forwarder plugin '{0}' to {1}", info.Name, info.LocalZipFileLocation));

                        if (this.errorMessage == null)
                        {
                            // unzip files to the correct location
                            string folder = Path.Combine(ForwardDestinationManager.UserPluginDirectory, Growl.CoreLibrary.PathUtility.GetSafeFolderName(info.Name));
                            if (!ApplicationMain.HasProgramLaunchedYet || !Directory.Exists(folder))
                            {
                                Utility.WriteDebugInfo(String.Format("Forwarder '{0}' downloaded - starting unzip.", info.Name));
                                Unzipper.UnZipFiles(info.LocalZipFileLocation, folder, false);

                                string text = String.Format(Properties.Resources.ForwarderInstaller_InstalledText, info.Name);
                                InternalNotification n = new InternalNotification(Properties.Resources.ForwarderInstaller_InstalledTitle, Utility.GetResourceString(text), null);
                            ...
                            else
                            {
                                ...
                                ShowMessage(String.Format(Utility.GetResourceString(Properties.Resources.ForwarderInstaller_AlreadyInstalled), info.Name), true);
                            }
                        ...
                        {
                            Utility.WriteDebugInfo(String.Format("Error downloading forwarder plugin '{0}'.", info.Name));
                ...
                else
                {
                    ...
                    ShowMessage(String.Format(Utility.GetResourceString(Properties.Resources.ForwarderInstaller_BadDefinitionFile), this.uri), true);
                }
            }
            catch (Exception ex)
            {
                // error downloading definition file
                Utility.WriteDebugInfo(String.Format("Error downloading forwarder plugin. {0} - {1}", ex.Message, ex.StackTrace));
                ShowMessage(String.Format(Utility.GetResourceString(Properties.Resources.ForwarderInstaller_NonexistentDefinitionFile), this.uri), true);
            }
            return installed;
        }
        ...
        {
            ForwarderInfo info = (ForwarderInfo)obj;
        ...
        void wc_DownloadFileCompleted(object sender, AsyncCompletedEventArgs e)
        {
            if (e.Error != null)
            {
                ...
                this.errorMessage = Utility.GetResourceString(Properties.Resources.ForwarderInstaller_DownloadError);
            }
            else if (e.Cancelled)
            {
                this.errorMessage = Utility.GetResourceString(Properties.Resources.ForwarderInstaller_DownloadCancelled);
            }
            else
            {
                // sometimes the downloaded file is still being written to disk.
                // this will wait until the file is readable before returning.
                ForwarderInfo info = (ForwarderInfo)e.UserState;
        ...
        public class ForwarderInfo
        {
            ...
            public ForwarderInfo(string name, string author, string version, string description, string packageUrl)
            {
                this.Name = name;
                this.Author = author;
            ...
            public static ForwarderInfo Parse(string data)
            {
                try
                {
                    ...
                    ForwarderInfo info = new ForwarderInfo(name, author, version, description, packageUrl);
                    return info;
                }
                catch
                {
